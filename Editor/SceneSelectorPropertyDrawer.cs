    ...
    public class SceneSelectorPropertyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (property.propertyType == SerializedPropertyType.String)
            {
                ...
                var scene = EditorGUI.ObjectField(position, label, sceneObject, typeof(SceneAsset),
                    true);
                if (scene == null)
                {
        ...
        private static SceneAsset GetSceneObject(string sceneObjectName)
        {
            ...
            foreach (var editorScene in EditorBuildSettings.scenes)
            {
                ...
                {
                    return AssetDatabase.LoadAssetAtPath(editorScene.path, typeof(SceneAsset)) as
                        SceneAsset;
                }
            }
